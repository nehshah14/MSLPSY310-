...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Asus\\OneDrive\\Desktop\\MSL\\motor sequence_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    fixation = visual.ShapeStim(
        ...
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor=[0.6078, -0.2784, -0.2784],
    ...
    fixations = visual.ShapeStim(
        win=win, name='fixations', vertices='cross',units='pix', 
        size=(10, 10),
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    line3_ = visual.Rect(
        ...
        opacity=None, depth=-3.0, interpolate=True)
    line4_ = visual.Rect(
        win=win, name='line4_',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(150, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    probe_ = visual.ShapeStim(
        win=win, name='probe_',units='pix', 
        size=(10, 10), vertices='triangle',
        ori=0.0, pos=[0,0], draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('OneDrive/Documents/keys.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial_2 in trials_2:
        ...
        trial2 = data.Routine(
            name='trial2',
            components=[fixations, line, line2_, line3_, line4_, probe_, key_resp_2],
        )
        trial2.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        probe_.setPos((pos, 60))
        ...
        while continueRoutine:
            ...
            # if fixations is starting this frame...
            if fixations.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fixations.frameNStart = frameN  # exact frame index
                fixations.tStart = t  # local t and not account for scr refresh
                fixations.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fixations, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fixations.started')
                # update status
                fixations.status = STARTED
                fixations.setAutoDraw(True)
            
            # if fixations is active this frame...
            if fixations.status == STARTED:
                # update params
                pass
            
            # if fixations is stopping this frame...
            if fixations.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixations.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    fixations.tStop = t  # not accounting for scr refresh
                    fixations.tStopRefresh = tThisFlipGlobal  # on global time
                    fixations.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fixations.stopped')
                    # update status
                    fixations.status = FINISHED
                    fixations.setAutoDraw(False)
            
            # *line* updates
            
            # if line is starting this frame...
            if line.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line.started')
                # update status
                line.status = STARTED
            ...
            # if line2_ is starting this frame...
            if line2_.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line2_.frameNStart = frameN  # exact frame index
                line2_.tStart = t  # local t and not account for scr refresh
                line2_.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line2_, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line2_.started')
                # update status
                line2_.status = STARTED
                line2_.setAutoDraw(True)
            
            # if line2_ is active this frame...
            if line2_.status == STARTED:
                # update params
                pass
            
            # *line3_* updates
            
            # if line3_ is starting this frame...
            if line3_.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line3_.frameNStart = frameN  # exact frame index
                line3_.tStart = t  # local t and not account for scr refresh
                line3_.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line3_, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line3_.started')
                # update status
                line3_.status = STARTED
                line3_.setAutoDraw(True)
            
            # if line3_ is active this frame...
            if line3_.status == STARTED:
                # update params
                pass
            
            # *line4_* updates
            
            # if line4_ is starting this frame...
            if line4_.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line4_.frameNStart = frameN  # exact frame index
                line4_.tStart = t  # local t and not account for scr refresh
                line4_.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line4_, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line4_.started')
                # update status
                line4_.status = STARTED
                line4_.setAutoDraw(True)
            
            # if line4_ is active this frame...
            if line4_.status == STARTED:
                # update params
                pass
            
            # *probe_* updates
            
            # if probe_ is starting this frame...
            if probe_.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                probe_.frameNStart = frameN  # exact frame index
                probe_.tStart = t  # local t and not account for scr refresh
                probe_.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(probe_, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'probe_.started')
                # update status
                probe_.status = STARTED
                probe_.setAutoDraw(True)
            
            # if probe_ is active this frame...
            if probe_.status == STARTED:
                # update params
                pass
